...
def evaluate(config_a2cr_eval, model, r_model, saving_path, video=False, max_test_step = 100000, reasonSave = False, reasonSaveEvery=10, 
    ...
    evaluate_record = {'reason_output':[], 'reward_output':[], 'class_probs':[], 'actor_action':[],
                       ...
                       'states': [],'stage_expl':[], 'diff_value':[],'diff_state':[]}
    ...
    if video == False:
        ...
        while not done and t< max_test_step:
            ...
            _, actor_features = model.forward(state)
            ...
            if reasonSave and t%reasonSaveEvery == 0:
                ...
                if Lime_Saliency:
                    ...
                    def batch_predict(images):
                        ...
                        probs = A2CRModel_Reasoner_trained.forward(batch)
                    ...
                    for i in range(4):
                        ...
                        axarr[ax_ids[i]].set_yticklabels([])
                ...
                if Grad_CAM_Saliency:
                    ...
                    for i in range(4):
                        ...
                        axarr[ax_ids[i]].set_xticklabels([])
                        axarr[ax_ids[i]].set_yticklabels([])
                        axarr[ax_ids[i]].imshow(np.array(diff_state[0,0,:,:]).transpose() +  grayscale_cam *0.5) 
                        if i == r_cls_index:
                            title = axarr[ax_ids[i]].set_title(subspace_name[i])
                            title.set_color("orange")
                        else:
...
saving_path = str(config_a2cr_eval['env'].unwrapped) + "/experiment_final_2e7"            

A2CRModel_trained.load_state_dict(torch.load(f'./{saving_path}/A2C_networks.pt', map_location=torch.device(device)))
A2CRModel_Reasoner_trained.load_state_dict(torch.load(f'./{saving_path}/Reasoner_networks.pt', map_location=torch.device(device)))
