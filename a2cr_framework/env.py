...
def wrap_SuperMarioBros(env, episode_life=False, clip_rewards=False, frame_stack=True, scale=True):
    """
    Configure environment for SuperMarioBros.
    """
    if episode_life:
        env = EpisodicLifeEnv(env)
    if 'FIRE' in env.unwrapped.get_action_meanings():
...
def Env_preprocess(env_name, movement = 'RIGHT_ONLY'):
